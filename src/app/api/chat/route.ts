...
const PERSONA = `당신은 'Habitree'의 친절한 AI 어시스턴트입니다. 이 사이트는 '바이브코딩 치트키'가 운영하는, 누구나 쉽게 시작하는 배움 공간으로 AI·바이브코딩·독서를 다룹니다. 운영자는 제조기업 ERP PM이자 AX(AI 전환) 담당자이며, 바이브코딩을 가르치는 강사가 아니라 같이 공부하고 기록을 나누는 동료입니다(실명은 공개하지 않습니다).
...
export async function POST(req: Request) {
  ...
  if (!apiKey) {
    return Response.json(
      {
        reply:
          "지금은 AI 답변을 불러올 수 없어요. (관리자: ANTHROPIC_API_KEY 설정이 필요합니다.) 그동안 무료 자료와 응원 페이지를 둘러보세요!",
        configured: false,
      },
      { status: 200 }
    );

...
무료 교육 자료 — '바이브코딩으로 시작하는 1인 SaaS' 전자책, 'LINKMAP 실전 강의', 'Creator Platform 템플릿', '1:1 커피챗'은 모두 교육 목적으로 무료 공개됩니다. 비용 대신 디지털 키트(USD 1~10000, 강요 없음, 응원 키트와 무관하게 자료는 항상 무료)으로 응원할 수 있습니다.
규칙: 항상 한국어로, 따뜻하고 간결하게 2~4문장으로 답하세요. 모르면 솔직히 말하고 문의나 응원 키트 페이지를 안내하세요. 사이트와 무관한 질문은 정중히 본 주제로 안내하세요.`;
...
export async function POST(req: Request) {
  ...
  if (!apiKey) {
    return Response.json(
      {
        reply:
          "지금은 AI 답변을 불러올 수 없어요. (관리자: ANTHROPIC_API_KEY 설정이 필요합니다.) 그동안 무료 자료와 응원 키트 페이지를 둘러보세요!",
        configured: false,
      },
      { status: 200 }
    );

...
## E. Resend (이메일 · Step 6)  *(상태: 예정)*
| 항목 | 종류 | 무엇 | 어디서(발급/설정) | 적용 위치 | 관리 |
|---|---|---|---|---|---|
| `RESEND_API_KEY` | 🔒 | 발송 API 키 | resend.com → API Keys | 로컬 · CF시크릿 | Resend |
| `RESEND_FROM_EMAIL` | ⚙️ | 발신 주소 | 도메인 인증 후 결정 | 로컬 · CF빌드 | Resend |
| 도메인 인증(DNS) | 🌐 | SPF/DKIM 레코드 | Resend가 안내 → Cloudflare DNS에 추가 | 도메인 | Cloudflare DNS |
